...
.DESCRIPTION
  ...
  - Visar resultatet i Out-GridView
...
try {
    [System.Xml.XmlDocument]$xml = Get-Content -Path $xmlPath -Encoding UTF8 -ErrorAction Stop
...
# ------------------------------------------------------------------------------------
# Steg 5: Slå ihop AD och ClientHealth-data
# ------------------------------------------------------------------------------------
$results = foreach ($computer in $inactiveComputers) {
    $ch = $clientHealthData | Where-Object { $_.Hostname -eq $computer.Name }
...
# ------------------------------------------------------------------------------------
# Steg 7: Visa i Out-GridView
# ------------------------------------------------------------------------------------
#$results | Out-GridView -Title "Inaktiva datorer (OU: Win11\Inaktiva) - ClientHealth-data"

# ------------------------------------------------------------------------------------
# Steg 8: Exportera till CSV (-ExportCSV eller -SendMail)
# CSV genereras alltid när -SendMail används så den kan bifogas mailet
# ------------------------------------------------------------------------------------
$csvFile = $null
...
# ------------------------------------------------------------------------------------
# Steg 9: HTML-rapport (valfritt, -HTMLReport eller -SendMail)
# ------------------------------------------------------------------------------------
$htmlFile    = $null
$htmlContent = $null
...
# ------------------------------------------------------------------------------------
# Steg 10: Skicka e-post med HTML + CSV som bilagor (valfritt, -SendMail, ej DryRun)
# ------------------------------------------------------------------------------------

$logoBase64 = "
...
if ($SendMail) {
    ...
    if ($isDryRun) {
        ...
        # Bygg lista över bilagor — alltid HTML, CSV om den skapades
...
<body>
<img alt="My image" src="data:image/png;base64,$logoBase64" style="height:60px; display:block; margin-bottom:16px;">
  <h2>$reportTitle</h2>
