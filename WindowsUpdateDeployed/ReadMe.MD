...
- 📅 **Automated Scheduling** - Runs on specified days after Patch Tuesday
- 📧 **Email Reports** - Sends HTML-formatted update reports via SMTP
- 📝 **Configurable Logging** - Automatic log rotation based on file size
- 🔧 **XML Configuration** - Easy configuration without modifying script code
- 🚫 **Flexible Reporting** - Option to disable reports for specific months
- 🎨 **Professional HTML Output** - Styled HTML tables with update details
...
<Configuration>
  <!-- MECM Site Server -->
  <SiteServer>SITESERVERNAME.domain.com</SiteServer>
  
  <!-- Email Configuration -->
  <MailSMTP>smtp.company.com</MailSMTP>
  <MailPort>25</MailPort>
  <Mailfrom>noreply@company.com</Mailfrom>
  <MailCustomer>Company Name</MailCustomer>
  
  <!-- Recipients -->
  <Recipients>
    <Recipients>
      <Email>admin1@company.com</Email>
    </Recipients>
    <Recipients>
      <Email>admin2@company.com</Email>
    </Recipients>
    <Recipients>
      <Email>team@company.com</Email>
    </Recipients>
  </Recipients>
  ...
  <Logfile>
    <Path>C:\Logs\</Path>
    <Name>windowsupdatedeployed.log</Name>
    <Logfilethreshold>5242880</Logfilethreshold> <!-- 5 MB in bytes -->
  </Logfile>
  
  <!-- Update Deployment Settings -->
  <UpdateDeployed>
    <LimitDays>-30</LimitDays>
    <DaysAfterPatchToRun>7</DaysAfterPatchToRun>
    <UpdateGroupName>2024-* - All Software Updates</UpdateGroupName>
  </UpdateDeployed>
  
  <!-- Disable Report for Specific Months -->
  <DisableReportMonth>
    <DisableReportMonth>
      <Number>7</Number>  <!-- July -->
    </DisableReportMonth>
    <DisableReportMonth>
      <Number>12</Number> <!-- December -->
    </DisableReportMonth>
  </DisableReportMonth>
</Configuration>
```
...
1. ✅ Check if the current month is in the disabled months list - **Exit if disabled**
2. ✅ Calculate Patch Tuesday for the current month
3. ✅ Determine if today equals Patch Tuesday + configured days
4. ✅ If today matches:
   - Query MECM for updates in the specified Software Update Group
   - Generate HTML report with update details
   - Send email to configured recipients
5. ✅ Log all activities to the log file
6. ✅ Rotate log file if it exceeds the size threshold
