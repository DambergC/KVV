...
.DESCRIPTION
   ...
   This version is configured via XML (no parameters) and generates the report with PSWriteHTML.
   ...
   Expected XML file: Send-DPStatus.xml (next to script)

   Additional optional XML:
   <Configuration>
     <General>
       <DryRun>true</DryRun>
     </General>
...
#########################################################
$xmlPath = Join-Path -Path $PSScriptRoot -ChildPath 'Send-CheckDPStatus.XML'
...
$DryRun = Get-XmlBool -Node $xml -Path '/Configuration/General/DryRun' -Default $false

$SiteServer    = Get-XmlText -Node $xml -Path '/Configuration/SiteServer'
$DPMaintGroup  = Get-XmlText -Node $xml -Path '/Configuration/DPGroups/Maintenance'
$DPProdGroup   = Get-XmlText -Node $xml -Path '/Configuration/DPGroups/Production'

$ReachabilityMethod = (Get-XmlText -Node $xml -Path '/Configuration/Reachability/Method')
if (-not $ReachabilityMethod) { $ReachabilityMethod = 'ICMP' }

$PingCount = Get-XmlInt -Node $xml -Path '/Configuration/Reachability/PingCount' -Default 1

$MailSMTP      = Get-XmlText -Node $xml -Path '/Configuration/Mail/SMTP'
$MailPortnumber = Get-XmlInt -Node $xml -Path '/Configuration/Mail/Port' -Default 25
$UseSecureConnectionIfAvailable = Get-XmlBool -Node $xml -Path '/Configuration/Mail/UseSecureConnectionIfAvailable' -Default $false
$MailFrom      = Get-XmlText -Node $xml -Path '/Configuration/Mail/From'
$SubjectPrefix = Get-XmlText -Node $xml -Path '/Configuration/Mail/SubjectPrefix'
if (-not $SubjectPrefix) { $SubjectPrefix = 'DP Status' }

$CustomerName  = Get-XmlText -Node $xml -Path '/Configuration/Report/CustomerName'
if (-not $CustomerName) { $CustomerName = 'Customer' }

$Logfile       = Get-XmlText -Node $xml -Path '/Configuration/Logging/Logfile'
if (-not $Logfile) { $Logfile = "G:\Scripts\Logfiles\DPLogfile.log" }

# Recipients from XML
$recipientNodes = $xml.SelectNodes('/Configuration/Mail/Recipients/Recipient')
$MailTo = @()
foreach ($r in @($recipientNodes)) {
    $email = $null
    if ($r.Attributes['email']) { $email = $r.Attributes['email'].Value }
    if (-not $email) {
        $email = Get-XmlText -Node $r -Path './Email'
    }
    if ($email -and $email.Trim() -ne '') { $MailTo += $email.Trim() }
}
...
foreach ($required in @(
    ...
    @{ Name = 'DPMaintGroup'; Value = $DPMaintGroup },
    @{ Name = 'DPProdGroup'; Value = $DPProdGroup },
    @{ Name = 'MailSMTP'; Value = $MailSMTP },
...
if ($MailTo.Count -eq 0) {
    throw "No recipients found in XML at /Configuration/Mail/Recipients/Recipient"
}
...
#########################################################
# SiteCode helper (WMI/DCOM to avoid WinRM/WSMan issues)
#########################################################
function Get-CMSiteCode {
    ...
    try {
        $prov = Get-WmiObject -Namespace 'root\SMS' -Class SMS_ProviderLocation -ComputerName $ComputerName |
...
Write-Log -LogString "$scriptname - Script start (DryRun=$DryRun)"
...
foreach ($dp in $dpNames) {
    $online = $false
    switch ($ReachabilityMethod.ToUpperInvariant()) {
        'ICMP' { $online = Test-Connection -ComputerName $dp -Count $PingCount -Quiet -ErrorAction SilentlyContinue }
        default { throw "Unsupported Reachability/Method '$ReachabilityMethod'. Supported: ICMP" }
    }
    ...
    foreach ($failedDP in $failedDPs) {
        ...
        try {
            ...
            if (-not $isInMaintenance) {
                if ($DryRun) {
                    Write-Log -LogString "DRYRUN: $failedDP offline -> would enable Maintenance Mode and move to '$DPMaintGroup' (remove from '$DPProdGroup')"
                    $resultOfflineActions.Add((New-ResultRow -Name $failedDP -Description $desc -Status 'Offline' -Action 'DRYRUN (no change)' -Notes "Would enable maintenance + move to '$DPMaintGroup'")) | Out-Null
                } else {
                    ...
                    $resultOfflineActions.Add((New-ResultRow -Name $failedDP -Description $desc -Status 'Offline' -Action "Enabled maintenance; moved to '$DPMaintGroup'" -Notes 'DP not reachable')) | Out-Null
                }
            } else {
...
if ($succeededDPs.Count -gt 0) {
    ...
    foreach ($okDP in $succeededDPs) {
        ...
        try {
            ...
            if ($isInMaintenance) {
                if ($DryRun) {
                    Write-Log -LogString "DRYRUN: $okDP online + maintenance -> would disable Maintenance Mode and move to '$DPProdGroup' (remove from '$DPMaintGroup')"
                    $resultRestoreActions.Add((New-ResultRow -Name $okDP -Description $desc -Status 'Online' -Action 'DRYRUN (no change)' -Notes "Would disable maintenance + move to '$DPProdGroup'")) | Out-Null
                } else {
                    ...
                    $resultRestoreActions.Add((New-ResultRow -Name $okDP -Description $desc -Status 'Online' -Action "Disabled maintenance; moved to '$DPProdGroup'" -Notes 'DP reachable again')) | Out-Null
                }
            } else {
...
#########################################################
# Convert typed lists to arrays safely
$offlineRows  = @($resultOfflineActions.ToArray())
$restoreRows  = @($resultRestoreActions.ToArray())
$noChangeRows = @($resultNoChange.ToArray())
$errorRows    = @($resultErrors.ToArray())

# Offline list (ALL offline by ping) - Name, Description, Status
$offlineListRows = foreach ($failedDP in $failedDPs) {
    $desc = ''
    try {
        $dpInfo = Get-CMDistributionPointInfo -SiteSystemServerName $failedDP
        $desc = $dpInfo.Description
...
$reportTitle = "$CustomerName - Kontroll av Distributionspunkter"
...
zSji7zpenNgf/tgSGIN1aQfmCXIG5AbhZED5ez8Niphl3rvnzgA0kneSsAlaQNMCcCXzoOva7wIGp/24MwQpDXQGeWyf8pkvzGdQEcTJwZp7qXWmE/wettNO/1iYB3AAAAABJRU5ErkJggg=="'
    ...
    New-HTMLSection -HeaderText "Summary" -HeaderTextSize 20 -HeaderBackGroundColor Darkblue {
        New-HTMLTag -Tag 'div' -Attributes @{ class = 'summary' } {
            ...
            New-HTMLTag -Tag 'div' { "Antal Distributionspunkter: <b>$TotalDPscount</b>" }
            New-HTMLTag -Tag 'div' { "Antal Distributionspunkter som svarar på ping: <b>$SucceededDPscount</b>" }
            New-HTMLTag -Tag 'div' { "Antal Distributionspunkter som inte svarar på ping: <b>$FailedDpscount</b>" }
            New-HTMLTag -Tag 'div' { "DryRun: <b>$DryRun</b>" }
        }
    }

    # Offline list (ALL offline by ping) - Name, Description, Status
    New-HTMLSection -HeaderText "Offline DPs (Ping failed) ($(@($offlineListRows).Count))" -HeaderTextSize 20 -HeaderBackGroundColor Darkblue {
        if (@($offlineListRows).Count -eq 0) { New-HTMLText -Text 'Inga offline DPs.' -Color Gray }
        else { New-HTMLTable -DataTable ($offlineListRows | Select-Object Name, Description, Status) }
    }


    New-HTMLSection -HeaderText "Offline DPs - Modified ($($offlineRows.Count))" -HeaderTextSize 20 -HeaderBackGroundColor Darkblue {
        if ($offlineRows.Count -eq 0) { New-HTMLText -Text 'Inga poster.' -Color Gray }
        else { New-HTMLTable -DataTable ($offlineRows | Select-Object Timestamp, Name, Description, Status, Action, Notes) }
    }

    New-HTMLSection -HeaderText "Online DPs - Restored ($($restoreRows.Count))"  -HeaderTextSize 20 -HeaderBackGroundColor Darkblue{
        if ($restoreRows.Count -eq 0) { New-HTMLText -Text 'Inga poster.' -Color Gray }
        else { New-HTMLTable -DataTable ($restoreRows | Select-Object Timestamp, Name, Description, Status, Action, Notes) }
    }

    <##
    New-HTMLSection -HeaderText "Ingen ändring ($($noChangeRows.Count))" -HeaderTextSize 20 -HeaderBackGroundColor Darkblue {
        if ($noChangeRows.Count -eq 0) { New-HTMLText -Text 'Inga poster.' -Color Gray }
        else { New-HTMLTable -DataTable ($noChangeRows | Select-Object Timestamp, Name, Description, Status, Action, Notes) }
    }
    ##>

    New-HTMLSection -HeaderText "Error ($($errorRows.Count))" -HeaderTextSize 20 -HeaderBackGroundColor Darkblue {
        if ($errorRows.Count -eq 0) { New-HTMLText -Text 'Inga fel.' -Color Gray }
        else { New-HTMLTable -DataTable ($errorRows | Select-Object Timestamp, Name, Description, Status, Action, Notes) }
    }
...
$Subject = [string]"$SubjectPrefix - $today"
