...
$ouMoveMap = @(
    ...
    [pscustomobject]@{
        ...
        TargetOU = "OU=T1,OU=Win11,OU=Inaktiva,OU=Datorer,OU=Kriminalvården,DC=kvv,DC=se"
                    ...
                    Write-Log -Message "Inaktiverad: $($computer.Name)"
$excludedOUs = @(
    ...
    "OU=DP-Server,OU=T1,OU=Fysiska klienter,OU=Windows 11,OU=Datorer,OU=Kriminalvården,DC=kvv,DC=se"
                    Write-Log -Message "Inaktivering misslyckades: $($computer.Name): $($_.Exception.Message)" -Level 'ERROR'
)
$emailRecipients = "HK.IT.CMKalendern@kriminalvarden.se"
                Write-Log -Message "Redan inaktiverad: $($computer.Name)" -Level 'WARN'
...
$logFile          = Join-Path -Path $logRoot -ChildPath ("Inaktivera_flytta_datorer_{0}.log" -f (Get-Date -Format "yyyyMMdd"))
# =========================================
# Funktioner
# =========================================
...
$processedResults = @()
try {
    Write-Log -Message "Startar kontroll. SOKBAS: $searchBase. Simulation: $simulationMode"
    # Validera mål-OU:er
    ...
    Write-Log -Message ("Hittade {0} datorobjekt under SOKBAS." -f $computerCount)
    if ($computerCount -gt $maxObjectsPerRun) {
        Write-Log -Message ("Avbryter: antal ({0}) overstiger maxgrans ({1})." -f $computerCount, $maxObjectsPerRun) -Level 'ERROR'
        ...
        Write-Log -Message ("Klar. Totalt: {0}, nyinaktiverade: {1}, redan inaktiverade: {2}, nyflyttade: {3}, fel: {4}." -f $retrievedCount, $disabledNowCount, $alreadyDisabledCount, $movedNowCount, $failedItems)
        exit 1
    }
                ...
                if ($isInactiveForMove) {
                    if ($computer.DistinguishedName -like "*,$resolvedTargetOU") {
                        ...
                        try {
                            ...
                            $moveStatus = "Success"
                            Write-Log -Message "Flyttad: $($computer.Name) -> $resolvedTargetOU"
                        }
                        catch {
                            $moveStatus = "Failed"
                            ...
                            Write-Log -Message "Flytt misslyckades: $($computer.Name) -> $resolvedTargetOU: $($_.Exception.Message)" -Level 'ERROR'
                        }
                    }
                }
                else {
                    $moveStatus = "SkippedBecauseComputerIsNotInactive"
                    Write-Log -Message "Flytt skippad (inte inaktiv): $($computer.Name)" -Level 'WARN'
                }
            }
        }
        else {
